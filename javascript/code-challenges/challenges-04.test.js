...
const updateAnimal = (arr, callback) => {
  ...
  arr.map(str => outputArr.push(callback(str)));
...
const sortByPrice = (arr) => {
  return arr.sort((prod1, prod2) => prod1.price - prod2.price);
};
...
const alphabetizeBetter = (arr) => {
  const lowered = arr.map((str, i) => {
    return {i, lowerCase: str.toLowerCase()};
  });

  lowered.sort((a,b) => {
    return a.lowerCase > b.lowerCase ? 1 : a.lowerCase < b.lowerCase ? -1 : 0;
  });

  let output = lowered.map(val => arr[val.i]);
...
const sortNumbersByLength = (arr) => {
  const mapped = arr.map((num, i) => {
    return {i, value: num.toString()};
  });

  mapped.sort((a, b) => a.value.length - b.value.length);

  return mapped.map((obj => arr[obj.i]));
};
