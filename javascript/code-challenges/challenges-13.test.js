...
const longestString = (arr) => {
  if (arr.length > 0) {
    let result = arr.reduce((acc, val, idx) => val.length > acc[0] ? [val.length, idx] : acc, [0, 0]);
    return result[1];
  } else { return -1; }
};
...
const standardizePhoneNumbers = (arr) => {
  return arr.map(str => str.replaceAll(/\D/g, ''));
};
...
const allHappy = (arr) => {
  return arr.reduce((acc, val) => val.includes(':)') ? acc : acc=false, true);
};
...
const findEvery = (arr, target) => {
  return arr.reduce((acc, val) => val.includes(target) ? acc : acc=false, true);
};
...
const unenrollBrook = (arr) => {
  return arr.map(arr2 => arr2.filter(str => !str.includes('Brook')));
};
