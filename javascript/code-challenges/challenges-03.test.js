...
const getBaseStatGreaterThan = (arr, minBaseStat) => {
  return arr.filter(data => data.baseStat > minBaseStat);
};
...
const getStatName = (arr, minBaseStat) => {
  ...
  return filteredStats.map(data => data.stat.name);
};
...
const getCharactersWithoutChildren = (arr) => {
  return arr.filter(char => {
    let qualities = [];
    for (let about in char){
      qualities.push(about);
    }
    return !qualities.includes('children');
  });
};
...
const evenOddNumericValues = (arr) => {
  let numArr = arr.filter(val => typeof(val) === 'number');
  let evenOddArr = numArr.map(num => {
    if (!(num % 2)){
      return 'even';
    }else if(num % 2){
      return 'odd';
